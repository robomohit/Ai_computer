...
- **Status:** done (2026-05-19: added _buildTurnTimeline() in app.js — on first expand, hides raw C1 tool cards and builds an icon-gutter step timeline (one row per tool action) with .turn-step-icon / .turn-step-label / .turn-step-sub / .turn-step-output; long output is height-capped at 260px with overflow:auto; CSS in style.css; test_phase_c2_step_timeline_present added)
...
### [IDEA-2026-05-19-11] Add copy button to turn-step-output blocks

- **Source / context:** Phase C2 implementation 2026-05-19 — `.turn-step-output` renders long terminal output in a 260px height-capped scrollable block. Users cannot easily copy the full output; the existing Copy log button copies the whole task log, not one step.
- **Why it fits Ai_computer:** The step timeline is the primary tool-inspection surface. One-click copy on each output block (same pattern as existing .copy-log button) makes debugging faster.
- **Scope (this PR only):** In `_buildTurnTimeline()` in `app.js`, when a `.turn-step-output` block is created, append a small hover-revealed copy button. On click: `navigator.clipboard.writeText(out.textContent)` + `showToast('Copied', 'ok')`. CSS: hover-only, top-right of block. ~15 LOC JS + 5 LOC CSS.
